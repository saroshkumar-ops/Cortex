...
def project_prediction_from_context(ctx: Context | None, incident_id: str | None) -> dict | None:
    ...
    if ctx is None:
        return None
    # cascade_path = services touched by the causal chain (best-effort from related_events)
    event_map = {
        ev.get("event_id"): ev
        for ev in (ctx.get("related_events") or [])
        if isinstance(ev, dict) and ev.get("event_id") is not None
    }
    cascade_services: list[str] = []
    for edge in ctx.get("causal_chain", []):
        for eid_key in ("cause_event_id", "effect_event_id"):
            ...
            try:
                ...
                continue
            ev = event_map.get(eid_int)
            if not ev:
                continue
            svc_raw = ev.get("service") or ev.get("target")
            if not svc_raw:
                continue
            if svc_raw not in cascade_services:
                cascade_services.append(svc_raw)
    return {
