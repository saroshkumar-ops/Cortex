...
TEMPORAL DRIFT ADAPTABILITY:
  - Events are bucketed into time windows relative to signal (0-30s, 30-180s, ...)
    so incidents firing at different latencies (90s vs 600s post-deploy) can still
    share coarse "pre" tokens.
  - Metrics are tiered (low/mid/high) rather than exact values, tolerating metric
    baseline drift over time.
  - Log message templates are parameter-invariant; two "timeout calling X (req_id=Y)"
    logs land on the same template, reducing token variance.

CROSS-SERVICE DETECTION:
  - When an error log mentions another service (e.g., "timeout calling billing-svc"),
    emit `cross_service_error:role:time` token to signal cascade.
  - When traces span multiple services, emit `trace_cross_service:role:time`.
  - These high-signal tokens get boosted during reranking, helping identify correlated failures.
"""
...
def build_tokens(engine, incident_id: str) -> tuple[set[str], dict]:
    ...
    for eid in window_ids:
        ...
        if kind == "deploy":
            ...
            # Detect cross-service spans (spans not in canonical_svc)
            cross_svc_spans = []
            self_touched = False
            for sp in spans:
                ...
                if sp_canonical == canonical_svc and canonical_svc:
                    self_touched = True
                elif sp_canonical and sp_canonical != canonical_svc:
                    cross_svc_spans.append(sp_canonical)
            
            if self_touched:
                for b in time_labels:
                    tokens.add(f"trace_touches_self:{b}")
            
            # High-signal token: cross-service trace patterns indicate cascade
            if cross_svc_spans:
                for b in time_labels:
                    tokens.add(f"trace_cross_service:{role}:{b}")
            
            # Detect trace errors/timeouts if error info is present
            if ev.get("error"):
                for b in time_labels:
                    tokens.add(f"trace_error:{role}:{b}")
        elif kind == "topology":
            for b in time_labels:
